...
namespace Renderer.Modeling
{
    public static class MeshShapeGenerator<T> where T : struct, IVertex<PositionNormal>
    {
            ...
            })
            {
                ...
                if (dirX.x != 0)
                {
                    ...
                    if (dirY.y != 0)
                    {
                        if (length(trans) == 0)
                        {
                            ...
                            if (holeXYDown)
                            {
                                face = MyManifold<PositionNormal>.MiddleHoleSurface(slices, stacks, sepXYDown.Value);
                            }
                        }
                        else if (!faceXYUp)
                            continue;
                        ...
                        {
                            face = MyManifold<PositionNormal>.MiddleHoleSurface(slices, stacks, sepXYUp.Value);
                        }
                    }
                    else if (dirY.z != 0)
                    {
                        if (length(trans) == 0)
                        {
                            ...
                            if (holeXZDown)
                            {
                                face = MyManifold<PositionNormal>.MiddleHoleSurface(slices, stacks, sepXZDown.Value).ApplyTransforms(
                        ...
                        {
                            face = MyManifold<PositionNormal>.MiddleHoleSurface(slices, stacks, sepXZUp.Value).ApplyTransforms(
                            Transforms.RotateX(- pi / 2));
                        }
                    }
                }
                else
                {
                    ...
                    if (length(trans) == 0)
                    {
                        ...
                        if (holeYZDown)
                        {
                            face = MyManifold<PositionNormal>.MiddleHoleSurface(slices, stacks, sepYZDown.Value).ApplyTransforms(
                    ...
                    {
                        face = MyManifold<PositionNormal>.MiddleHoleSurface(slices, stacks, sepYZUp.Value).ApplyTransforms(
                            Transforms.RotateY(pi/2));
                    }
                }
                if (face == null)
        ...
        public static Mesh<PositionNormal> Cylinder(int points, float thickness=0, float angle = 2 * pi, bool surface = false)
        {
            ...
            var face1 = MyManifold<PositionNormal>.Revolution(ss, ss, x => float3(1 * x, 0, 0), float3(0, 0, 1), angle).Transform(Transforms.Translate(0,0,.5f));
            var face2 = MyManifold<PositionNormal>.Revolution(ss, ss, x => float3(1 * x, 0, 0), float3(0, 0, 1), angle).Transform(Transforms.Translate(0,0,-.5f));
            
            if (thickness != 0)
            {
                baseCylOuter = MyManifold<PositionNormal>.Revolution(ss, ss, x => float3(1 + thickness, 0, x), float3(0, 0, 1), angle).Transform(Transforms.Translate(0, 0, -.5f));
                face1 = MyManifold<PositionNormal>.Revolution(ss, ss, x => float3(1 + x * thickness, 0, 0), float3(0, 0, 1), angle).Transform(Transforms.Translate(0, 0, .5f));
                face2 = MyManifold<PositionNormal>.Revolution(ss, ss, x => float3(1 + x * thickness, 0, 0), float3(0, 0, 1), angle).Transform(Transforms.Translate(0, 0, -.5f));
            }
            var baseCyl = MyManifold<PositionNormal>.Revolution(ss, ss, x => float3(1, 0, x), float3(0,0,1), angle).Transform(Transforms.Translate(0,0,-.5f));
            if (surface)
            {
                return face1;
            }

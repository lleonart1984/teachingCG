...
namespace Renderer
{
    class Program
    {
        ...
        static Mesh<MyVertex>[] CreateModel()
        {
            ...
            Mesh<MyVertex> coffee_base_mesh = CoffeMakerSection_Mesh(buttonBasePoints, topBasePoints);
            Mesh<MyVertex> base_mesh = Mesh_Poliedro(buttonBasePoints, float3(0, 0, 0));
            ...
            Mesh<MyVertex> union_mesh = CoffeMakerSection_Mesh(buttonUnionPoints, topUnionPoints);
            ...
            Mesh<MyVertex> top_mesh = CoffeMakerTopSection_Mesh(buttonTopPoints, topTopPoints, 2.1f, 0.7f);
            ...
            Mesh<MyVertex> tapa_mesh = CoffeMakerSection_Mesh(buttonTapaPoints, topTapaPoints);
            ...
            Mesh<MyVertex> cosita_mesh = CoffeMakerSection_Mesh(buttonCositaPoints, topCositaPoints);
            Mesh<MyVertex> top_cosita_mesh = Mesh_Poliedro(topCositaPoints, float3(0, altura_cosita + h_cosita, 0));
            cosita_mesh = cosita_mesh.Add_Mesh(top_cosita_mesh);


            Mesh<MyVertex> up_mesh = top_mesh.Add_Mesh(handle_mesh).Add_Mesh(tapa_mesh).Add_Mesh(cosita_mesh);
            up_mesh = up_mesh.Transform(Transforms.RotateRespectTo(float3(0,0,0), float3(0,1,0), pi/3));


            Mesh<MyVertex> r_model = base_mesh.Add_Mesh(coffee_base_mesh).Add_Mesh(union_mesh).Add_Mesh(up_mesh);
        ...
        private static Mesh<MyVertex> Mesh_Poliedro(List<float3> poli, float3 center)
        {
            MyVertex[] vertices = new MyVertex[poli.Count + 1];
            int[] indices = new int[3 * poli.Count];

            vertices[0] = new MyVertex{Position = center};

            int j = 0;
            int i = 1;
            for(; i < poli.Count; i++)
            {
                vertices[i] = new MyVertex{Position = poli[i - 1]};
        ...
        private static Mesh<MyVertex> CoffeMakerTopSection_Mesh(List<float3> baseF, List<float3> topF, float r, float d)
        {
            int n = baseF.Count;
            ...
            MyVertex[] vertices = new MyVertex[baseF.Count + topF.Count + 4];
            ...
            vertices[j] = new MyVertex{Position = baseF[baseF.Count - 1]};
            vertices[j+1] = new MyVertex{Position = topF[topF.Count - 1]};
            vertices[j+2] = new MyVertex{Position = a};
            ...
            vertices[j+3] = new MyVertex{Position = b};
            vertices[j+4] = new MyVertex{Position = p};
            vertices[j+5] = new MyVertex{Position = q};
